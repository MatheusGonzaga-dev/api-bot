...
const getExecutableDir = () => {
    return __dirname; // Aponta diretamente para o diretório atual, assumindo que os arquivos estão no mesmo nível
};
...
app.get('/creddeb', (req, res) => {
    ...
    console.log(`Tentando acessar o arquivo JSON em: ${jsonFilePath}`);
    if (fs.existsSync(jsonFilePath)) {
        const data = fs.readJsonSync(jsonFilePath);
        res.json(data);
    } else {
        console.error(`Arquivo JSON de CRED E DEB não encontrado em: ${jsonFilePath}`);
...
app.get('/blocos', (req, res) => {
    ...
    if (fs.existsSync(jsonFilePath)) {
        ...
        console.error(`Arquivo JSON de BLOCOS não encontrado em: ${jsonFilePath}`);
